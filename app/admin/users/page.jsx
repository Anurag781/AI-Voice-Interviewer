    ...
    try {
      ...
      const usersWithStats = await Promise.all(
        data.map(async (user) => {
          ...
          // Count all candidates from all interviews
          if (interviews && interviews.length > 0) {
            const interviewIds = interviews.map((item) => item.interview_id);
  ...
  const exportUsersToCSV = () => {
    ...
    const rows = filteredUsers.map((user) => {
      const createdDate = user.created_at
        ? moment(String(user.created_at)).format("YYYY-MM-DD HH:mm")
        : "N/A";
    ...
    link.download = `users-${moment().format("YYYY-MM-DD")}.csv`;
